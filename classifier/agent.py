...
from core.agents import extract_text_with_openai
...
def classify_files_in_folder(
    ...
    def _process_single_file(src_path: str) -> Optional[Dict[str, Any]]:
        """Process a single file: returns result dict or None."""
        filename = os.path.basename(src_path)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_src):
            ...
            if result["type"] == "skipped":
                ...
                person_counts[result["person_name"]] = person_counts.get(result["person_name"], 0) + 1
                copied.append(
                    {
                        "source": result["filename"],
                        "person_name": result["person_name"],
                        "doc_type_en": result["doc_type_en"],
                        "to": result["to"],
                    }
                )
